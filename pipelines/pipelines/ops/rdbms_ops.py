...
def full_loads_with_batch(
    ...
    def full_loads_table():
        ...
        if len(columns) == 1 and columns[0] == "*":
            ...
            datas = rdbmsObj.full_load_table_with_spesific_columns_with_batch(
                tb_name, columns, 1000
            )

        result["datas"] = datas
        final_result = (result, tb_name, db_destination)
        return clickhouseSvc.insert_datas_from_mysqldatas_to_clickhouse(
            name, final_result, db_sources, chServer
        )
