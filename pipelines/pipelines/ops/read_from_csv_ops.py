...
def full_loads_csv(
    ...
    def load_datas():
        downloaded_datas = csv_from_urls(url) if source_kind == "url" else csv_from_file(path_file)
        result,headers = next(downloaded_datas)
        result = result[1:]

        cleanned_headers = [datas for datas in headers][0]
        cleanned_headers = ["_".join(column.replace(".","").strip().split(" ")).lower() for column in cleanned_headers]
        columns = ",".join([ "{} {}".format(column,"String") for column in cleanned_headers ])
